...
const SPRITE_SHEET = '/Users/whitman/Desktop/BubbleBros/lib/images/sprite_file.png';
...
export default class Player extends MovingObject {
  ...
  collideWith(otherObject, direction) {
    if (otherObject instanceof Bubble && !this.dying) {
      this.collisionAnimation(direction)
      setTimeout(() => {
        if (this.livesLeft > 0) {
          this.livesLeft -= 1;
          this.resetPlayerPos();
    ...
    if (time - this.lastFire > GRAPPLE_TIMEOUT) {
      ...
      setTimeout(() => {
        const grapple = new Grapple({
          pos: [this.pos[0] + 27, this.pos[1] + 35],
          game: this.game,
        });
